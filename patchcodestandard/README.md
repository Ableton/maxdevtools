...
# Version control
* The device code and its dependencies live unfrozen in its repository.
...
* Only use an existing abstraction if it does exactly what you need and not more. If you need something slightly different, de-encapsulate and change it.
...
* For polyphonic patches, `[poly~]`s only contain the calculations that are different per voice, e.g. conversions and smoothing that are the same for all voices can be done outside. This doesn't apply when `[poly~]` is used for different purposes, e.g. to resample parts of the audio chain.
* For muting parts of DSP processing, when there are no other demands, using `[mute~]` is preferable over `[poly~]`.
...
* We try to avoid having parameters in repeated abstractions, including in bpatchers, since those parameters are auto-renamed by Max.
...
* In final versions of devices, there are no comments with "TODO" in them, todos should be done.
